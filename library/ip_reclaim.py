...
def main(ip_address,result):
    try:
        ...
        dns_names = ip_object[0]["names"]
        a_record = ip_object[0]["objects"]
        ip_state = ip_object[0]["status"]
        if ip_state == 'USED':
            if len(dns_names) <= 1 or len(a_record) <= 1:
                  ...
                  mesg=ip_address + "- There are more than 1 record found.Please get confirmation from user."
        else:
          err_msg="Ip status is UNUSED."
